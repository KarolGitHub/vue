<template>
  ...
  color === 'light' ? 'text-blueGray-600' : 'text-white',
            ...
            <td class="border-t-0 px-6 align-middle border-l-0 border-r-0 text-xs whitespace-nowrap p-4">
              <div class="flex">
                <img v-for="(team, index) in item.users" :src="team" :key="`user-${index}`" alt="..."
  ...
  'bg-emerald-500': item.completion === 100
      ...
      <Paginator :pagination="pagination" :class="[color === 'light' ? 'bg-white' : 'bg-emerald-900 text-white']"
        @onNext="onChangePage($event)" @onPrevious="onChangePage($event)" @onSelectPage="onChangePage($event)"
        @onFilteredData="onFilterPagination($event)" :data="filteredList" />
    </div>
  </div>
</template>
